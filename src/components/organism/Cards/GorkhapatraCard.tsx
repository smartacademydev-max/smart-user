    ...
    return (
        <Box className="px-1.5 pt-1.5 pb-3 rounded-md h-full flex flex-col justify-between" sx={{
            ...
            <div className="top__wrapper">
                <Box className="image__wrapper aspect-316/160 rounded-md overflow-hidden relative flex flex-col justify-center items-center" sx={{
                    ...
                    {data?.thumbnail_url ? <img src={data?.thumbnail_url} alt={data?.title} className='w-full h-full object-cover' /> : <>
                        ...
                        <Typography variant='body2' color='info.main' className='text-center px-4 line-clamp-2'>{data?.title}</Typography>
                    </>}
                ...
                <div className="content__box pt-2 px-2">
                    <Link to={PATH.GORKHAPATRA.VIEW_GORKHAPATRA.ROOT(Number(data.id))}>
                        <Typography sx={{
                            fontSize: "12.5px", fontWeight: 700, color: "text.primary",
                            lineHeight: 1.4, mb: "5px",
                            display: "-webkit-box", WebkitLineClamp: 2,
                            WebkitBoxOrient: "vertical", overflow: "hidden",
                            "&:hover": { color: (theme) => theme.palette.primary.dark }
                        }}>{data.title}</Typography>
                    </Link>
                    <Typography sx={{ fontSize: "11px", fontWeight: 500, color: "text.secondary" }}>{data.title}</Typography>
                </div>
            </div>
            <div className="bottom px-2">
                ...
                <Stack className='justify-between mb-4'>
                    <Stack className='items-center! gap-1!'>
                        <Calendar size={16} />
                        <Typography sx={{ fontSize: "11px", color: "text.secondary" }}>{date}</Typography>
                    </Stack>
                    <Stack className='items-center! gap-1!'>
                        <Eye size={16} />
                        <Typography sx={{ fontSize: "11px", color: "text.secondary" }}>{data?.views} {t("messages.views")}</Typography>
                    </Stack>
                </Stack>
