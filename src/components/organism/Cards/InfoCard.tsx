...
export default function InfoCard({ title, description, icon, cta }: InfoCardProps) {
    return (
        <Box className="info-card p-4 lg:p-8 rounded-lg  " sx={{
            bgcolor: (theme) => theme.palette.primary.contrastText,
            border: (theme) => `1px solid ${theme.palette.separator.dark}`
        }}>
            {icon &&
                <Box className="icon mb-4 lg:mb-5 max-w-14 min-h-15 aspect-square rounded-full flex items-center justify-center" sx={{
