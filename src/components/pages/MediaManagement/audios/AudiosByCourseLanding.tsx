...
function AudiosSection({ course }: { course: CourseProps }) {
    const navigate = useNavigate();
    const { data, isLoading } = useGetCourseMediaPlaylistQuery(
        { id: course.id!, type: "audios", qp: { pageIndex: 1, pageSize: PREVIEW_PAGE_SIZE } },
        { skip: !course.id }
    );
    const items = data?.data?.data || [];
    const total = data?.data?.pagination?.total ?? items.length;
    const viewAllUrl = PATH.AUDIOS.BY_COURSE.ROOT(course.id);
    ...
    return (
        ...
        >
            {isLoading ? (
                ...
                <div className="text-sm text-gray-500 italic py-4">No audios yet for this course.</div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4">
                    ...
                    {extra > 0 && (
                        <ViewAllTile extra={extra} url={viewAllUrl} label="audios" variant="playlist" />
                    )}
                </div>
            )}
        </CourseSectionShell>
...
export default function AudiosByCourseLanding() {
    ...
    return (
        <>
            <PageHeader breadcrumb={[{ title: t("messages.all_audios") }]} />
            {isLoading ? (
                ...
                <>
                    <div className="flex flex-col">
                        {courses.map((course) => (
                            <AudiosSection key={course.id} course={course} />
                        ))}
                    </div>
