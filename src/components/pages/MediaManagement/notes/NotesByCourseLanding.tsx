...
function NotesSection({ course }: { course: CourseProps }) {
    const navigate = useNavigate();
    const { data, isLoading } = useGetCourseMediaPlaylistQuery(
        { id: course.id!, type: "notes", qp: { pageIndex: 1, pageSize: PREVIEW_PAGE_SIZE } },
        { skip: !course.id }
    );
    const items = data?.data?.data || [];
    const total = data?.data?.pagination?.total ?? items.length;
    const viewAllUrl = PATH.NOTES.BY_COURSE.ROOT(course.id);
    ...
    return (
        ...
        >
            {isLoading ? (
                ...
                <div className="text-sm text-gray-500 italic py-4">No notes yet for this course.</div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-3 md:gap-4">
                    ...
                    {extra > 0 && (
                        <ViewAllTile extra={extra} url={viewAllUrl} label="notes" variant="playlist" />
                    )}
                </div>
            )}
        </CourseSectionShell>
...
export default function NotesByCourseLanding() {
    ...
    return (
        <>
            <PageHeader breadcrumb={[{ title: t("messages.all_notes") }]} />
            {isLoading ? (
                ...
                <>
                    <div className="flex flex-col">
                        {courses.map((course) => (
                            <NotesSection key={course.id} course={course} />
                        ))}
                    </div>
