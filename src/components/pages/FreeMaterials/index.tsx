...
export default function FreeMaterials() {
    ...
    const skip = !freeCourseId;

    const { data: videosRes, isLoading: loadingVideos } = useGetCourseMediaByTypeQuery(
        { id: freeCourseId, type: "videos", qp: videoQp },
        { skip }
    );
    const { data: notesRes, isLoading: loadingNotes } = useGetCourseMediaByTypeQuery(
        { id: freeCourseId, type: "notes", qp: notesQp },
        { skip }
    );
    const { data: audiosRes, isLoading: loadingAudios } = useGetCourseMediaByTypeQuery(
        { id: freeCourseId, type: "audios", qp: audiosQp },
        { skip }
    );
    const { data: testsRes, isLoading: loadingTests } = useGetCourseTestQuery(
        { id: freeCourseId!, ...testsQp },
        { skip }
    );
    const { data: liveRes, isLoading: loadingLive } = useGetCourseLiveClassQuery(
        { id: freeCourseId!, ...liveQp },
        { skip }
    );
    ...
    if (!loadingFreeCourse && !freeCourseId) {
        return (
            <div className="h-full overflow-auto pr-4 pb-8">
                <EmptyList
                    title="No Free Materials configured"
                    description="No open-access course is set up yet. Please contact your administrator."
                />
            </div>
        );
    }
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
                                {loadingVideos
                                    ...
                                    : videoItems.map((m: MediaProps) => (
                                        <MediaCard key={m.id} media={m} type="temp_video" havePurchased={true} courseId={freeCourseId} />
                                    ))
                                }
                            </div>
                        </section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
                                {loadingNotes
                                    ...
                                    : notesItems.map((m: MediaProps) => (
                                        <MediaCard key={m.id} media={m} type="temp_notes" havePurchased={true} courseId={freeCourseId} />
                                    ))
                                }
                            </div>
                        </section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
                                {loadingAudios
                                    ...
                                    : audiosItems.map((m: MediaProps) => (
                                        <MediaCard key={m.id} media={m} type="temp_audios" havePurchased={true} courseId={freeCourseId} />
                                    ))
                                }
                            </div>
                        </section>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-3">
                                {loadingLive
                                    ...
                                    : liveItems.map((l: LiveClassProps) => (
                                        <LiveClassCard key={l.id} data={l} courseId={freeCourseId ?? undefined} />
                                    ))
                                }
                            </div>
                        </section>
